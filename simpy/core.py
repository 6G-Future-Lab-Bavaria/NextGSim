"""
Core components for event-discrete ran_simulation environments.
...
class StopSimulation(Exception):
    """Indicates that the ran_simulation should stop now."""

    @classmethod
    def callback(cls, event):
        """Used as callback in :meth:`BaseEnvironment.run()` to stop the
        ran_simulation when the *until* event occurred."""
        if event.ok:
...
class Environment(BaseEnvironment):
    """Execution environment for an event-based ran_simulation. The passing of time
    ...
    @property
    def now(self):
        """The current ran_simulation time."""
        return self._now
