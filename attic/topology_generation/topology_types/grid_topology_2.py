...
def create_random_grid_topology():
    ...
    gateway = Gateway(location=[np.random.randint(0, grid_size_length), np.random.randint(0, grid_size_width)])
    ...
    for i in range(num_of_edge_servers):
        edge_server_list.append(EdgeServer(location=[np.random.randint(0, grid_size_length), np.random.randint(0, grid_size_width)]))
        add_link(source=gateway, destination=edge_server_list[i])
        add_link(source=edge_server_list[i], destination=gateway)

    for i in range(num_of_rows):
        for j in range(num_of_cols):
            base_station_links_list.append(add_link(source=base_station_list[i][j], destination=gateway))
            base_station_links_list.append(add_link(source=gateway, destination=base_station_list[i][j]))

            # if i != num_of_rows - 1 and j != num_of_cols - 1:
            #     base_station_links_list.append(
            #         add_link(source=base_station_list[i][j], destination=base_station_list[i + 1][j]))
            #     base_station_links_list.append(
            #         add_link(source=base_station_list[i + 1][j], destination=base_station_list[i][j]))
            #     base_station_links_list.append(
            #         add_link(source=base_station_list[i][j], destination=base_station_list[i][j + 1]))
            #     base_station_links_list.append(
            #         add_link(source=base_station_list[i][j + 1], destination=base_station_list[i][j]))
            #     continue
            #
            # elif j == num_of_cols - 1 and i != num_of_rows - 1:
            #     base_station_links_list.append(
            #         add_link(source=base_station_list[i][j], destination=base_station_list[i + 1][j]))
            #     base_station_links_list.append(
            #         add_link(source=base_station_list[i + 1][j], destination=base_station_list[i][j]))
            #     continue
            #
            # elif i == num_of_rows - 1 and j != num_of_cols - 1:
            #     base_station_links_list.append(
            #         add_link(source=base_station_list[i][j], destination=base_station_list[i][j + 1]))
            #     base_station_links_list.append(
            #         add_link(source=base_station_list[i][j + 1], destination=base_station_list[i][j]))
    ...
    with open('../../config/config_test_3.json', 'w+') as json_file:
        try:
