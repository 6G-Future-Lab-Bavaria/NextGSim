...
class OffloadedData(Message):
    def __init__(self):
        super().__init__(name="OffloadedData",
                         source_service="DataGeneration",
                         msg_type="SOURCE",
                         destination_service="DataProcessingLoadBalancer",
...
class DataProcessing(Microservice):
    name = "DataProcessing"
    app_name = "RANApplicationPublicEnergy"
    user = "public"
    request_cpu_share = 1
    required_memory = 1
...
class RANApplicationPublicEnergy(Application):
    name = "RANApplicationPublicEnergy"
    services = [DataGeneration, DataProcessing]
    number_of_service_instances = {"DataProcessing": 10}
    ...
    def __init__(self, user_id=None):
        super().__init__(name="RANApplicationPublicEnergy")
        self.set_user_id(user_id)
        self.set_services(RANApplicationPublicEnergy.services,
                          RANApplicationPublicEnergy.number_of_service_instances)
        add_app(self)
