...
# class Server:
#     def __init__(sim):
#         sim.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
#         sim.SERVER_HOST = "131.159.192.148"
#         sim.SERVER_PORT = 9990
#         sim.BUFFER_SIZE = 4096
#         sim.SEPARATOR = "<SEPARATOR>"
#
#     def start_server(sim, sim):
#         global FILE_COUNTER_LOCAL
#         sim.socket.bind((sim.SERVER_HOST, sim.SERVER_PORT))
#         sim.socket.listen(5)
#         client_socket, addr = sim.socket.accept()
#         client_socket.send(START_SIG_MESSAGE)
#
#         print("Sent start signal")
#         received_msg = client_socket.recv(sim.BUFFER_SIZE).decode()
...
#                 try:
#                     received_msg = client_socket.recv(sim.BUFFER_SIZE).decode()
#                     filename, filesize, file_counter = received_msg.split(sim.SEPARATOR)
#                     print("filename")
#                     print(filename)
#                     print("filesize")
#                     print(filesize)
...
#                             # print("Waiting bytes")
#                             bytes_read = client_socket.recv(sim.BUFFER_SIZE)
#                             if bytes_read.decode() == COMPLETION:
#                                 print("File Transfer Completed")
#                                 # nothing is received_msg
#                                 # file transmitting is done
#                                 break
#                             # write to the file the bytes we just received_msg
#                             # print("BYTES READ")
#                             # print(bytes_read)
#                             f.write(bytes_read)
...
#                     print("IM HERE")
#
#                     yield sim.env.timeout(sim.ran_csv_read_distribution.next())
#
#
...
class Server(object):
    ...
    def handshake(self):
        # sim.s.send(sim.START_SIG.encode())
        dataFromClient = self.s.recv(1024)
        print(dataFromClient.decode())
        if dataFromClient.decode() != self.START_SIG:
            return
        else:
            # dataFromClient = sim.s.recv(1024)
            # print(dataFromClient.decode())
            # if dataFromClient.decode() != sim.START_SIG:
                # return
    ...
    def receive_files(self):
        ...
        # received_TTI = sim.client_socket.recv(sim.BUFFER_SIZE).decode('utf-8')
