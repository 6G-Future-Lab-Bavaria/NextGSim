    ...
    def calc_achieved_throughput(self):
        for user in range(0, self.num_users):
            for BS in range(0, self.num_BSs):
                for PRB in range(0, self.num_PRBs):
                    # TODO: put the correct bandwidth for the user
...
# TODO: 2. Based on the received SINRs, we decide the max SINR for each user for each gNB and select the max SINR
#  between all gNBs. This will define with which gNB the user will be connected, regardless of the state. Here, check
...
#  Round Robin: Loop over all users; Loop over all PRBs; assign the selected PRB to the selected user ;
...
class Scheduler:
    ...
    def schedule_with_round_robin(self):
        """
        Each user gets PRBs allocated and his achievable rate is calculated.
        First, users get a quotient integer of PRBs, then remaining PRBs are allocated to users one by one.
        :param TTI:
        :return: Sets my_prbs for every connected user
        """
        # todo: reset my_prbs of non-connected users
        # todo: reserve resources for user in prepared cells; add len(gnb.prepared_cells)
        for user in self.simulation.devices_per_scenario:
            user.my_prbs = []  # reset
        ...
        for gnb in self.simulation.gNBs_per_scenario:
            ...
            for user in gnb.connected_devices:
                ...
                # print(f"User id {user.ID}, {user.handover_is_ongoing}, {user.handover_interruption_time}")
                # current_time = self.simulation.TTI * self.simulation.sim_params.TTI_duration
                # if user.hit_finish and user.hit_finish >= current_time:  # this user is making a handover
                #     user.my_prbs = []
                #     continue
        ...
        # for user in self.simulation.devices_per_scenario:
        #     if len(user.my_prbs) == 0:
        #         print(f"UE {user.ID} was not served at {self.simulation.TTI}")
    ...
    def visualize_scheduler_allocation(self, TTI):
        ...
        for device in self.simulation.devices_per_scenario:
            ...
            # sinr = self.simulation.channel.measured_SINR[device.ID, device.my_gnb.ID]
            ...
            if TTI in self.simulation.handover.who_made_handovers and device.ID in \
                ...
                label = 'Made handover'
                counter += 1
                # print(f"Plot user {device.ID} handover to gnb {device.my_gnb.ID}")
            else:
                color = '#0099FF'
                label = ''
    ...
    def check_prbs_assignment(self):
        ...
        for gNB in self.simulation.gNBs_per_scenario:
            ...
            for user in gNB.connected_devices:
                # print(f"User {user.ID} connected {user.my_gnb.ID} with {len(user.my_prbs)} PRBs")
                num_assigned_prbs_per_gnb += len(user.my_prbs)
            # print(f"At TTI={self.simulation.TTI}, gNB assigned {num_assigned_prbs_per_gnb} out of {gNB.available_resources}")
    ...
    def dummy_scheduler(self):
        ...
        for gnb in self.simulation.gNBs_per_scenario:
            ...
            for user in gnb.connected_devices:
                ...
                if user.my_rate != 0:
                    assert 0, "Why user's rate was not reset?"
